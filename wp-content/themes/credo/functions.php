 ...
 */
/**
 * Credo only works in WordPress 4.8 or later.
 */
...
function ftc_custom_sidebar_form() {
    ...
    <form action="<?php echo admin_url('widgets.php'); ?>" method="post" id="ftc-form-add-sidebar">
        <input type="text" name="sidebar_name" id="sidebar_name" placeholder="<?php esc_html_e('Custom Sidebar Name', 'credo') ?>" />
        <button class="button-primary" id="ftc-add-sidebar"><?php esc_html_e('Add Sidebar', 'credo') ?></button>
    </form>
    <?php
}
...
$file_names = array('instagram','feedburner_subscription', 'products', 'blogs', 'blogs_tabs', 'recent_comments', 'product_categories','product_filter_by_color');
foreach ($file_names as $file) {
...
function ftc_setup() {
    ...
    register_nav_menus(array(
        'primary' => __('Primary Navigation', 'credo'),
        'vertical' => __('Vertical Navigation', 'credo'),
    ));
   ...
   add_editor_style( array( 'assets/css/editor-style.css', ftc_fonts_url() ) );
    ...
    $starter_content = array(
        ...
        'attachments' => array(
            'image-espresso' => array(
                'post_title' => _x('Espresso', 'Theme starter content', 'credo'),
                'file' => 'assets/images/espresso.jpg', // URL relative to the template directory.
            ),
            'image-sandwich' => array(
                'post_title' => _x('Sandwich', 'Theme starter content', 'credo'),
                'file' => 'assets/images/sandwich.jpg',
            ),
            'image-coffee' => array(
                'post_title' => _x('Coffee', 'Theme starter content', 'credo'),
        ...
        'nav_menus' => array(
            ...
            'social' => array(
                'name' => __('Social Links Menu', 'credo'),
                'items' => array(
                    'link_yelp',
                    'link_facebook',
                    'link_twitter',
    ...
    /**
     * Filters Credo array of starter content.
     *
     * @since Credo 1.1
     *
...
function ftc_content_width() {
    ...
    /**
     * Filter Credo content width of the theme.
     *
     * @since Credo 1.0
     *
...
function ftc_fonts_url() {
     ...
     */
    $dosis = _x( 'on', 'Poppins, Raleway font: on or off', 'credo' );

    if ( 'off' !== $dosis ) {
        $font_families = array();

        $font_families[] = 'Poppins:400,500,600,700|Raleway:200,300,400,500,600,700,800';
...
function ftc_excerpt_more($link) {
    ...
    $link = sprintf('<p class="link-more"><a href="%1$s" class="more-link">%2$s</a></p>', esc_url(get_permalink(get_the_ID())),
            /* translators: %s: Name of current post */ sprintf(__('Continue reading<span class="screen-reader-text"> "%s"</span>', 'credo'), get_the_title(get_the_ID()))
    );
    return ' &hellip; ' . $link;
}
...
wp_enqueue_script('cookie', get_template_directory_uri().'/assets/js/jquery.cookie.min.js', array( 'jquery' ), null, true );
    ...
    if (has_nav_menu('top')) {
        ...
        $ftc_l10n['expand'] = __('Expand child menu', 'credo');
        $ftc_l10n['collapse'] = __('Collapse child menu', 'credo');
...
if (!function_exists('ftc_get_search_form_by_category')) {
    ...
    function ftc_get_search_form_by_category() {
        ...
        if ($search_for_product) {
            ...
            $post_type = 'product';
            $placeholder_text = esc_html__('Search ...', 'credo');
        } else {
            $taxonomy = 'category';
            $post_type = 'post';
            $placeholder_text = esc_html__('Search', 'credo');
        ...
        $options = '<option value="">' . esc_html__('All categories', 'credo') . '</option>';
...
if (!function_exists('ftc_ajax_search')) {
    ...
    function ftc_ajax_search() {
        ...
        if ($results->have_posts()) {
            ...
            if (isset($results->post_count, $results->found_posts) && $results->found_posts > $results->post_count) {
                $view_all_text = sprintf(esc_html__('View all %d results', 'credo'), $results->found_posts);
...
if (!function_exists('ftc_breadcrumbs')) {
    ...
    function ftc_breadcrumbs() {
        ...
        if (!empty($front_id)) {
            ...
            $home = esc_html__('Home', 'credo');
        }
        $ar_title = array(
            'search' => esc_html__('Search results for ', 'credo')
            , '404' => esc_html__('Error 404', 'credo')
            , 'tagged' => esc_html__('Tagged ', 'credo')
            , 'author' => esc_html__('Articles posted by ', 'credo')
            , 'page' => esc_html__('Page', 'credo')
            , 'portfolio' => esc_html__('Portfolio', 'credo')
        );
...
function ftc_breadcrumbs_title($show_breadcrumb = false, $show_page_title = false, $page_title = '', $extra_class_title = '') {
    ...
    if ($show_breadcrumb || $show_page_title) {
        ...
        echo '<div class="ftc-breadcrumb" ' . $style . '><div class="ftc-breadcrumb-title">';
        if ($show_page_title) {
