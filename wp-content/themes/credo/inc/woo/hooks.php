...
function ftc_product_get_availability(){
    ...
    if ( ! $product->is_in_stock() ) {
        $availability = esc_html__( 'Out of stock', 'credo' );
    } elseif ( $product->managing_stock() && $product->is_on_backorder( 1 ) ) {
        $availability = esc_html__( 'Available on backorder', 'credo' );
...
function ftc_template_loop_categories(){
 global $product;        
    $categories_label = esc_html__('Categories: ', 'credo');
...
function ftc_template_product_size_chart_button() {
    ...
    $size_chart_image_product = get_post_meta($product->get_id(), 'ftc_prod_size_chart', true);
        ...
        if (!empty($size_chart_image_product) || !empty($size_chart_image_options)) {
            ...
            echo '<a class="ftc-size_chart" href="' . esc_url($ajax_url) . '"><i class="fa fa-table"></i> ' . esc_html__('Size Chart','credo') . '</a>';
        }
}
...
function ftc_shop_top_content_widget_area_button() {
    ...
    if (is_active_sidebar('product-category-top-content') && $smof_data['ftc_prod_cat_top_content']) {
        ?>
        <div class="prod-cat-show-top-content-button"><a href="#"><?php esc_html_e('Filter', 'credo') ?></a></div>
        <?php
    }
}
...
<?php
    ...
    <div class="social-sharing">
        <div class="print">
            <a href="javascript:window.print()" rel="nofollow"><i class="fa fa-print"></i><?php esc_html_e('Print', 'credo') ?></a>
        </div>
        <div class="email">
            <a href="mailto:?subject=<?php echo esc_attr(sanitize_title(get_the_title())); ?>&amp;body=<?php echo esc_url(get_permalink()); ?>">
                <i class="fa fa-envelope"></i>
    <?php esc_html_e('Email to a Friend', 'credo') ?>
            </a>
        </div>
...
function ftc_template_after_single_social_sharing() {
    ...
    function ftc_template_single_meta() {
       ...
       if ($smof_data['ftc_prod_cat']) {
            echo wc_get_product_category_list($product->get_id(), ', ', '<div class="caftc-link"><span>' . _n('Category:', 'credo', count($product->get_category_ids()), 'credo') . ' ', '</span><span class="cat-links"></span></div>');
        }
        if ($smof_data['ftc_prod_tag']) {
            echo wc_get_product_tag_list($product->get_id(), ', ', '<div class="tags-link"><span>' . _n('Tag:', 'Tags:', count($product->get_tag_ids()), 'credo') . ' ', '</span><span class="tag-links"></span></div>');
        }
    ...
    function ftc_template_single_sku() {
        ...
        if (wc_product_sku_enabled() && ( $product->get_sku() || $product->is_type('variable') )) {
            echo '<div class="sku-wrapper product_meta">' . esc_html__('Sku: ', 'credo') . '<span class="sku" itemprop="sku">' . (( $sku = $product->get_sku() ) ? $sku : esc_html__('N/A', 'credo')) . '</span></div>';
        }
    }
    ...
    function ftc_template_single_availability() {
        ...
        $product_stock = ftc_product_get_availability();
        $availability_text = empty($product_stock['availability']) ? esc_html__('In Stock', 'credo') : esc_attr($product_stock['availability']);
        ?>	
...
if (class_exists('YITH_WCWL')) {
    ...
    function ftc_add_wishlist_button_to_product_list() {
        ...
        . '" class="add_to_wishlist wishlist" ><i class="fa fa-heart"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'credo') . '</span></a>';
        ...
        echo '<a href="' . esc_url($wishlist_url) . '"><i class="fa fa-heart-o"></i><span class="ftc-tooltip button-tooltip">' . esc_html__('Wishlist', 'credo') . '</span></a>';
        echo '</span>';
...
if (class_exists('YITH_Woocompare') && get_option('yith_woocompare_compare_button_in_products_list') == 'yes') {
    ...
    if ($yith_woocompare->is_frontend() || $is_ajax) {
        ...
        add_action('woocommerce_after_shop_loop_item', 'ftc_add_compare_button_to_product_list', 81);
...
function ftc_cart_continue_shopping_button() {
    echo '<a href="' . esc_url(wc_get_page_permalink('shop')) . '" class="button button-secondary">' . esc_html__('Continue Shopping', 'credo') . '</a>';
}
?>