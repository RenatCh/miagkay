...
function ftc_custom_header_setup() {
	...
	/**
	 * Filter Credo custom-header support arguments.
	 *
	 * @since Credo 1.0
	 *
	 * @param array $args {
	...
	register_default_headers( array(
		'default-image' => array(
			...
			'description'   => __( 'Default Header Image', 'credo' ),
		),
	) );
}
...
function ftc_video_controls( $settings ) {
	$settings['l10n']['play'] = '<span class="screen-reader-text">' . __( 'Play background video', 'credo' ) . '</span>' . ftc_get_svg( array( 'icon' => 'play' ) );
	$settings['l10n']['pause'] = '<span class="screen-reader-text">' . __( 'Pause background video', 'credo' ) . '</span>' . ftc_get_svg( array( 'icon' => 'pause' ) );
	return $settings;
}
