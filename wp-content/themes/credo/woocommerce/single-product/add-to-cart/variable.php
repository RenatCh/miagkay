...
$attr_dropdown = !isset($smof_data['ftc_prod_attr_dropdown']) || ( isset($smof_data['ftc_prod_attr_dropdown']) && $smof_data['ftc_prod_attr_dropdown'] );
$select_class = '';
if( !$attr_dropdown ){
	$select_class = 'hidden';
}
...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->get_id() ); ?>" data-product_variations="<?php echo htmlspecialchars( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php esc_html_e( 'This product is currently out of stock and unavailable.', 'credo' ); ?></p>
	<?php else : ?>
				...
				<?php foreach ( $attributes as $attribute_name => $options ) : ?>
						...
						<div class="value">
							...
							<?php if( !$attr_dropdown && is_array( $options ) ): ?>
								<div class="ftc-product-attribute">
									<?php 
										...
										foreach ( $terms as $term ) {
											...
											if( $is_attr_color ){
												if( absint($datas['ftc_color_image']) > 0 ){
													echo '<a href="#">' . wp_get_attachment_image( absint($datas['ftc_color_image']), 'ftc_prod_color_thumb', true, array('title'=>$term->name, 'alt'=>$term->name) ) . '</a>';
												}
												else{
													echo '<a href="#" style="background-color:' . $datas['ftc_color_color'] . '">' . apply_filters( 'woocommerce_variation_option_name', $term->name ) . '</a>';
												}
											}
											else{
							...
							echo end( $attribute_keys ) === $attribute_name ? apply_filters( 'woocommerce_reset_variations_link', '<a class="reset_variations" href="#">' . esc_html__( 'Clear selection', 'credo' ) . '</a>' ) : '';
						?>
						</div>
                                            </div>
